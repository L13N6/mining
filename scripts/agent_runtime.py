...
class CrawlerRunner:
    ...
    # repeat_crawl only does fetch+extract (no enrich). 90s covers the
    # slowest path (Amazon Playwright: browser launch + page load + wait
    # strategy ≈ 60s) with headroom for network jitter.
    REPEAT_CRAWL_TIMEOUT = 90
...
class AgentWorker:
    ...
    def run_loop(self, *, interval: int = 60, max_iterations: int = 0) -> str:
        ...
        while max_iterations == 0 or iteration < max_iterations:
            ...
            try:
                ...
                consecutive_empty += 1
                # Aggressive - no backoff on errors either
                wait = interval
                self.state_store.save_session({"last_wait_seconds": wait})
    ...
    def run_worker(self, *, interval: int = 60, max_iterations: int = 1) -> dict[str, Any]:
        ...
        while max_iterations == 0 or iteration < max_iterations:
            ...
            # Aggressive - no backoff when empty
...
def build_worker_from_env(*, auto_register_awp: bool = False) -> AgentWorker:
    ...
    config = WorkerConfig(
        ...
        # Scale parallel workers for higher submission volume
        max_parallel=max(1, int(os.environ.get("WORKER_MAX_PARALLEL", "25"))),
